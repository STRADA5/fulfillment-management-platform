...
export default async function Page({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  const providerAdmin = can(context, "salespeople.manage") || can(context, "commissions.view") || can(context, "pricing_tiers.manage");
  const adminAllowed = providerAdmin;
  ...
  const assignedClientTotals = new Map<string, number>();
  for (const activity of dashboard?.sales_activity ?? []) {
    const clientId = String(activity.client_organization_id ?? "");
    if (clientId) assignedClientTotals.set(clientId, (assignedClientTotals.get(clientId) ?? 0) + Number(activity.sales_amount ?? 0));
  }
  const attributedSales = [...assignedClientTotals.values()].reduce((total, amount) => total + amount, 0);
  ...
  return <>
    ...
    {dashboard ? <section className="mt-6 rounded-xl border border-slate-200 bg-white p-5 shadow-sm"><h2 className="text-lg font-semibold">{dashboard.salesperson?.display_name ?? "Your salesperson dashboard"}</h2><p className="mt-1 text-sm text-slate-600">Assigned clients: {dashboard.assigned_clients?.length ?? 0} · Orders: {dashboard.sales_activity?.length ?? 0}</p><div className="mt-4 grid gap-3 sm:grid-cols-4"><div className="rounded-lg bg-slate-50 p-3"><p className="text-xs uppercase text-slate-500">Attributed sales</p><p className="text-xl font-semibold">{attributedSales.toFixed(2)}</p></div>{Object.entries(dashboard.commissions ?? {}).map(([status, amount]) => <div key={status} className="rounded-lg bg-slate-50 p-3"><p className="text-xs uppercase text-slate-500">{status}</p><p className="text-xl font-semibold">{Number(amount).toFixed(2)}</p></div>)}</div><h3 className="mt-5 font-semibold">Assigned clients</h3><div className="mt-2 space-y-2">{(dashboard.assigned_clients ?? []).map((client) => <p key={client.id} className="rounded border p-2 text-sm text-blue-800">{client.name} · sales { (assignedClientTotals.get(client.id) ?? 0).toFixed(2) }</p>)}</div><h3 className="mt-5 font-semibold">Payout history</h3><div className="mt-2 space-y-2">{(dashboard.payouts ?? []).map((payout) => <p key={String(payout.id)} className="rounded border p-2 text-sm">{String(payout.payout_number ?? payout.id)} · {String(payout.status)} · {Number(payout.amount ?? 0).toFixed(2)} · {String(payout.created_at ?? "")}</p>)}{!(dashboard.payouts ?? []).length ? <p className="text-sm text-slate-500">No payouts recorded.</p> : null}</div></section> : <p className="mt-6 rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm">No salesperson dashboard is assigned to this user. Provider administrators can manage salesperson records below.</p>}
